...
const StaffDashboard: React.FC = () => {
   ...
   const [view, setView] = useState<'meds' | 'safety' | 'agitation' | 'blackbox'>('agitation');
      ...
      <div className="min-h-screen bg-[#f1f5f9] text-[#1e293b] flex flex-col font-assistant" dir="rtl">
         ...
         <header className="bg-white border-b border-slate-200 p-6 flex justify-between items-center shadow-sm">
            ...
            <nav className="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
               ...
               <button onClick={() => setView('blackbox')} className={`px-6 py-2 rounded-lg text-xs font-black transition-all ${view === 'blackbox' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500'}`}>Black Box</button>
            </nav>
         </header>
            ...
            {view === 'blackbox' && (
               <div className="space-y-8 animate-in fade-in duration-700">
                  <div className="bg-slate-900 text-white p-12 rounded-[3.5rem] shadow-2xl relative overflow-hidden">
                     <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-red-600/10 blur-[100px] rounded-full" />
                     <div className="relative z-10 space-y-6">
                        <div className="inline-flex items-center gap-2 bg-red-500/10 text-red-500 px-4 py-1 rounded-full text-[10px] font-black uppercase border border-red-500/20">
                           <Radio size={12} className="animate-pulse" /> Secure Forensic Logs
                        </div>
                        <h2 className="text-5xl font-black italic tracking-tighter">הקופסה השחורה <span className="text-slate-600">|</span> <span className="text-red-500">תיעוד משבר</span></h2>
                        <p className="text-xl text-slate-400 font-medium max-w-2xl">
                           יומן אירועים מאובטח לניתוח והפקת לקחים (Debriefing). הצגת נתונים אובייקטיביים שנאספו במהלך אירועי קיצון.
                        </p>
                     </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                     {[
                        { id: 'LOG-4092', patient: 'עידן כהן', time: '02:14 AM', type: 'Panic Attack (SOS)', protocol: 'De-escalation Sequence', outcome: 'Resolved', duration: '4m 12s' },
                        { id: 'LOG-4091', patient: 'אמנון גבע', time: 'Yesterday, 23:40', type: 'Dopamine Storm', protocol: 'Auto-Lock (Financial)', outcome: 'Active', duration: 'N/A' },
                     ].map((log, i) => (
                        <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm flex items-center justify-between hover:border-blue-300 transition-all group">
                           <div className="flex items-center gap-6">
                              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white font-black shadow-lg ${log.outcome === 'Resolved' ? 'bg-slate-800' : 'bg-red-600'}`}>
                                 {i + 1}
                              </div>
                              <div>
                                 <h4 className="text-lg font-black text-[#1e40af]">{log.type}</h4>
                                 <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">{log.patient} • {log.time}</p>
                              </div>
                           </div>

                           <div className="flex items-center gap-8">
                              <div className="text-right hidden md:block">
                                 <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Protocol</p>
                                 <p className="text-sm font-bold text-slate-700">{log.protocol}</p>
                              </div>
                              <div className="text-right hidden md:block">
                              ...
                              <button className="bg-slate-100 text-slate-600 px-6 py-3 rounded-xl font-black text-xs uppercase hover:bg-blue-600 hover:text-white transition-all">
                                 פתח יומן מלא
                              </button>
                           </div>
                        </div>
