...
import {
   ...
   MessageCircle, Wind, ArrowDown, Clock, ShieldCheck, Lock
} from 'lucide-react';
      ...
      <div className="min-h-screen bg-slate-900 text-white flex flex-col font-sans overflow-hidden" dir="rtl">
         ...
         <header className="p-8 border-b border-white/5 flex justify-between items-center bg-slate-900/50 backdrop-blur-3xl z-50 shadow-2xl">
            <div className="flex items-center gap-6">
               ...
               <div>
                  <h1 className="text-2xl font-black italic tracking-tighter text-indigo-400">הכרטיס לחופש <span className="text-white font-light">Management</span></h1>
                  <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest">מערכת ניהול ובקרה (ROI Hub)</p>
               </div>
            </div>
         ...
         <main className="flex-1 p-10 overflow-y-auto bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-slate-800 to-slate-950 pb-32">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
               ...
               <div className="bg-slate-900 p-12 rounded-[4rem] border border-white/5 shadow-2xl space-y-10">
                  ...
                  <div className="p-8 bg-emerald-500/5 rounded-[3rem] border border-emerald-500/20">
                     <p className="text-emerald-400 font-black text-sm mb-2 uppercase tracking-widest">תובנת הנהלה (Decision Support):</p>
               ...
               {/* EMR Fusion Stream HUD */}
               <div className="bg-[#020617] p-12 rounded-[4rem] border border-indigo-500/30 shadow-[0_0_50px_rgba(79,70,229,0.1)] space-y-8 relative overflow-hidden group">
                  ...
                  <div className="flex justify-between items-center">
                     <div>
                        <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1">Strat-Fusion Engine</p>
                        <h3 className="text-2xl font-black italic flex items-center gap-3">
                           <Database className="text-indigo-400" />
                           Chameleon Data Stream (ADT/HL7)
                        </h3>
                     </div>
                     <div className="px-4 py-1 bg-white/5 rounded-full border border-white/10 text-[9px] font-bold text-slate-400 uppercase tracking-widest italic group-hover:text-indigo-400 transition-colors">
                        Integration Engine: Rhapsody v6.2
                     </div>
                  </div>

                  <div className="space-y-4">
                     {[
                        { type: 'ADT_A01', msg: 'Admission: Patient #812 (Ward B)', time: '14:42', color: 'indigo' },
                        { type: 'ORM_O01', msg: 'Order: Neurology Consultation (Z-Order)', time: '14:40', color: 'blue' },
                        { type: 'ORU_R01', msg: 'Result: Lab Panel Ready (Patient #422)', time: '14:38', color: 'emerald' },
                        { type: 'ADT_A03', msg: 'Discharge: Patient #109 (Home Flow)', time: '14:35', color: 'purple' },
                     ].map((msg, i) => (
                        <div key={i} className="flex items-center justify-between p-4 bg-white/5 border border-white/5 rounded-2xl hover:bg-indigo-500/10 transition-all border-l-4" style={{ borderColor: `var(--tw-color-${msg.color}-500)` }}>
                           <div className="flex items-center gap-4">
                              <span className="font-mono text-[10px] bg-white/5 px-2 py-1 rounded text-slate-400">{msg.type}</span>
                              <p className="text-sm font-bold text-white/90">{msg.msg}</p>
                           </div>
                           <span className="text-[10px] font-mono text-slate-500 italic">{msg.time}</span>
                        </div>
                     ))}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                     <div className="bg-white/5 p-6 rounded-3xl border border-white/5">
                        <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2 italic">Moat Strength Index</p>
                        <div className="flex items-end gap-2">
                           <span className="text-3xl font-black text-indigo-400 leading-none">88%</span>
                           <TrendingUp size={16} className="text-emerald-500 mb-1" />
                        </div>
                     </div>
                     <div className="bg-white/5 p-6 rounded-3xl border border-white/5">
                        <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2 italic">Readmission Prevention</p>
                        <div className="flex items-end gap-2 text-emerald-400">
                           <span className="text-3xl font-black leading-none">14.2%</span>
               ...
               {/* Security Audit Log */}
               <div className="bg-indigo-950/20 p-12 rounded-[4rem] border border-indigo-500/20 overflow-hidden shadow-2xl space-y-8">
                  <div className="flex justify-between items-center">
                     <div className="flex items-center gap-4">
                        <Lock className="text-indigo-400" />
                        <h3 className="text-2xl font-black">יומן אבטחה (Cyber Audit)</h3>
                     </div>
                     <div className="text-[10px] font-black uppercase text-indigo-500 tracking-widest">Zero-Trust Active</div>
                  </div>
                  <div className="overflow-x-auto">
                     <table className="w-full text-right">
                        <thead>
                           <tr className="text-[10px] text-slate-500 font-black uppercase tracking-widest border-b border-white/5">
                              <th className="px-4 py-4">זמן</th>
                              <th className="px-4 py-4">אירוע</th>
                              <th className="px-4 py-4">סטטוס</th>
                           </tr>
                        </thead>
                        <tbody className="divide-y divide-white/5">
                           {[
                              { time: '14:30', event: 'Access: Med Records', status: 'Success' },
                              { time: '14:28', event: 'Shield Active', status: 'Active' },
                              { time: '14:25', event: 'Access: Bio-Reactor', status: 'Blocked' },
                           ].map((log, i) => (
                              <tr key={i} className="hover:bg-indigo-500/5 transition-all text-sm">
                                 <td className="px-4 py-4 font-mono text-xs text-slate-400">{log.time}</td>
                                 <td className="px-4 py-4 font-bold">{log.event}</td>
                                 <td className="px-4 py-4">
                                    <span className={`px-3 py-1 rounded-full text-[9px] font-black tracking-widest uppercase ${log.status === 'Success' ? 'bg-emerald-500/10 text-emerald-500' :
            ...
            {/* Live Data Feed (Mock) */}
            <div className="mt-12 bg-white/5 rounded-[4rem] border border-white/5 overflow-hidden shadow-2xl mb-20">
