      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#e5ddd5] custom-scrollbar">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.sender === 'user' ? 'justify-start' : 'justify-end'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
            <div className={`max-w-[85%] p-3 rounded-xl shadow-sm text-xs font-medium whitespace-pre-wrap ${msg.sender === 'user' ? 'bg-[#dcf8c6] text-slate-800 rounded-tl-none ml-1' :
      ...
      <div className="space-y-4">
        {PROTOCOL_STEPS.map((step) => {
          ...
          return (
            <div
              key={step.id}
              className={`overflow-hidden rounded-[2.5rem] border-2 transition-all duration-500 ${isEmergency
            ...
            >
              ...
              >
                <div className="flex items-center gap-5">
                  <div className={`w-14 h-14 rounded-3xl flex items-center justify-center transition-all shadow-inner ${isEmergency ? 'bg-red-600 text-white animate-pulse' : 'bg-slate-50 text-slate-400'
                    }`}>
                    <Icon size={28} />
                  </div>
                  <div>
              ...
              {isExpanded && (
                <div className="px-8 pb-8 space-y-6 animate-in slide-in-from-top duration-500">
                  ...
                  <div className={`p-6 rounded-3xl border-2 flex items-start gap-4 ${isEmergency ? 'bg-red-100 border-red-200 text-red-900' : 'bg-blue-50 border-blue-100 text-blue-900'
                    }`}>
...
const CaregiverApp: React.FC<Props> = ({ onBack, ledgerEntries = [], onSendPoints }) => {
  const [activeTab, setActiveTab] = useState<'guide' | 'directory' | 'status'>('guide');
  const [showSaharChat, setShowSaharChat] = useState(false);
  const [scores, setScores] = useState<ClinicalScores>(ChameleonBridge.calculateClinicalScores('patient-001'));
  const [isSendingTouch, setIsSendingTouch] = useState(false);

  const handleSendTouch = () => {
    setIsSendingTouch(true);
    onSendPoints?.(10); // Reward the family for support
    setTimeout(() => setIsSendingTouch(false), 2000);
    // In a real system, this would trigger a socket/push event to the Patient App
  };
    ...
    <div className="max-w-md mx-auto bg-slate-50 min-h-screen shadow-2xl flex flex-col font-sans overflow-hidden border-x border-slate-200 pb-40" dir="rtl">
      ...
      <header className="p-6 bg-white border-b border-slate-100 flex justify-between items-center sticky top-0 z-50">
        <div className="flex flex-col">
          ...
          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">האפליקציה למשפחה</span>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setActiveTab('status')} className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all ${activeTab === 'status' ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
            <Activity size={20} />
          </button>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-50">
        {activeTab === 'guide' ? <FamilyCrisisGuide /> :
          activeTab === 'directory' ? <CaregiverDirectory /> :
            (
              <div className="p-6 space-y-6 animate-in slide-in-from-left duration-500">
                {/* Stability Status */}
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
                  <div className="flex justify-between items-center">
                    <h3 className="text-xl font-black italic">מה מצבו עכשיו?</h3>
                    <div className="flex items-center gap-2 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100">
                      <Database size={12} className="text-emerald-500" />
                      <span className="text-[9px] font-black text-emerald-500 uppercase">Chameleon Sync</span>
                    </div>
                  </div>

                  <div className="relative pt-8">
                    <div className="flex justify-between items-end mb-2">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">מדד יציבות (Stability)</p>
                      <p className="text-3xl font-black italic text-indigo-600">{scores.stabilityIndex}%</p>
                    </div>
                    <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                      <div className="h-full bg-indigo-500 transition-all duration-1000" style={{ width: `${scores.stabilityIndex}%` }} />
                    </div>
                    <p className="text-[9px] font-bold text-slate-500 mt-3 italic text-center">
                      המדד משתקלל בזמן אמת מתוך דיווחי הצוות ותוצאות המעבדה ב"קמיליון".
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-indigo-900 to-slate-900 p-8 rounded-[2.5rem] shadow-xl text-white relative overflow-hidden">
                  <div className="relative z-10 space-y-4">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="text-xl font-black italic">שלחו לו נגיעה</h3>
                    </div>
                    <p className="text-xs text-slate-300 font-medium leading-relaxed">
                      המטופל ירגיש דופק עדין במכשיר שלו ברגע שתלחצו. דרך שקטה לומר: "אני איתך".
                    </p>
                    <button
                      onClick={handleSendTouch}
                      disabled={isSendingTouch}
                      className={`w-full py-6 rounded-3xl font-black text-lg transition-all flex items-center justify-center gap-3 ${isSendingTouch ? 'bg-emerald-500 shadow-[0_0_30px_rgba(16,185,129,0.4)]' : 'bg-white text-slate-900 hover:bg-slate-100'
                        }`}
                    >
                      {isSendingTouch ? <Fingerprint className="animate-pulse" size={28} /> : <Heart size={28} />}
                      {isSendingTouch ? 'שלחנו לו!' : 'שלח נגיעה'}
                    </button>
                    {isSendingTouch && (
                      <p className="text-[10px] text-emerald-400 text-center font-black uppercase tracking-widest animate-fade-in">
                        +10 Support Points Earned
                      </p>
                    )}
                  </div>
                  <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-indigo-500/20 blur-3xl rounded-full" />
                </div>

                {/* Success Feed Preview */}
                <div className="space-y-4">
                  <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mr-2">הצלחות מהימים האחרונים</h4>
                  <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-center gap-4">
                    <div className="w-10 h-10 bg-emerald-100 text-emerald-600 rounded-2xl flex items-center justify-center">
                      <ShieldCheck size={20} />
                    </div>
                    <div>
                      <p className="text-[10px] text-slate-400 font-black">היום, 10:30 (פסיכולוג)</p>
                      <p className="text-sm font-black text-slate-800 italic leading-tight">"הפגין יוזמה גבוהה בשיחה הקבוצתית"</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/95 backdrop-blur-2xl border-t border-slate-100 p-6 z-50 shadow-[0_-15px_50px_rgba(0,0,0,0.12)]">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <button onClick={() => setActiveTab('directory')} className="flex flex-col items-center justify-center p-4 bg-slate-900 text-white rounded-[2rem] shadow-[0_8px_20px_rgba(15,23,42,0.3)] active:scale-95 transition-all group">
            ...
            <span className="text-[11px] font-black leading-none">צוות המחלקה</span>
          </button>
        </div>
      </div>
    </div>
