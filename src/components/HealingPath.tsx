...
const HealingPath: React.FC = () => {
   ...
   const startHealingConversation = async (topic: string) => {
      setActiveScreen('ai');
      setIsAiLoading(true);
      try {
   ...
   return (
      <div className="min-h-screen bg-[#0a0c10] text-white flex flex-col font-sans overflow-y-auto pb-40" dir="rtl">
         ...
         <header className="p-8 space-y-2 relative z-10">
            <h1 className="text-4xl font-black italic tracking-tighter">נתיב<span className="text-emerald-500 font-light italic"> ההחלמה</span></h1>
         ...
         <main className="flex-1 p-6 space-y-12 relative z-10">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {MEAL_STEPS.map((meal) => (
                     <button
                        ...
                        className={`flex items-center justify-between p-6 rounded-[2.5rem] border-2 transition-all group ${completedMeals.includes(meal.id) ? 'bg-emerald-600/10 border-emerald-500/40 shadow-inner' : 'bg-white/5 border-white/10 hover:border-emerald-500/20'
                           }`}
                     >
                        ...
                        <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all ${completedMeals.includes(meal.id) ? 'bg-emerald-500 border-emerald-500 text-white shadow-lg' : 'border-slate-700'
                           }`}>
            ...
            <section className="space-y-6">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <button
                     onClick={() => startHealingConversation('אני מרגישה חרדה גדולה אחרי הארוחה, המחשבות שלי רצות. תעזרי לי להירגע.')}
                  ...
                  <button
                     onClick={() => startHealingConversation('יש לי דחף חזק מאוד לאכול עכשיו בלי הפסקה, אני מרגישה שאיבדתי שליטה.')}
                     className="bg-white/5 border border-white/10 p-8 rounded-[3rem] text-right space-y-4 hover:bg-white/10 transition-all group"
                  >
         ...
         <div className="fixed bottom-8 left-8 right-8 max-w-md mx-auto z-50">
            <div className="bg-slate-900/90 backdrop-blur-2xl border border-white/10 p-6 rounded-[3rem] shadow-2xl flex justify-around items-center">
               ...
               <button onClick={() => startHealingConversation('ספרי לי משהו מרגיע ומחזק לקראת הארוחה הבאה שלי.')} className="text-slate-400 flex flex-col items-center gap-1 group">
